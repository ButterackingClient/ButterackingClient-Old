...
public abstract class EntityPlayer extends EntityLivingBase {
    ...
    private float eyeHeightTransitionSpeed = 0.008F;  // �� ���� ��ȭ �ӵ�

    public float getEyeHeight() {
        if (Client.I.hudManager.os.isEnabled()) {
            if (this.isPlayerSleeping()) {
                currentEyeHeight = 0.2F;
            }

            if (this.isSneaking()) {
                targetEyeHeight = 1.52F;
            } else {
                targetEyeHeight = 1.62F;
            }
            currentEyeHeight = lerp(currentEyeHeight, targetEyeHeight, eyeHeightTransitionSpeed);
        } else {

            if (this.isPlayerSleeping()) {
                currentEyeHeight = 0.2F;
            }else if (this.isSneaking()) {
                currentEyeHeight = 1.54F;
            } else {
                currentEyeHeight = 1.62F;
            }
        }
